...
class _AttractionTaskModel(BaseModel):
    ...
    agent: str = Field(default="attraction_agent")
...
def _dispatch_user_request_by_company(user_text: str) -> dict:
    prompt = PromptTemplate(
        template=(
            ...
            "10) budget 根据用户输入，默认 min=0,max=10000,currency=CNY。\n"
            "11) attraction_task 固定 task=search_attractions, agent=attraction_agent。\n"
...
def _build_flight_input(hotel_request_json: str) -> str:
    ...
    flight_request = {
        ...
        "budget": {
            ...
            "currency": "CNY",         # 从深圳出发，货币改为 CNY 比较合理，或保持原样
        },
    }
...
def run_test_main_agent_flow(user_text: str) -> dict:
    ...
    if isinstance(flight_result_outbound, dict):
        if "flight" in flight_result_outbound and flight_result_outbound["flight"]:
            ...
            merged_flight_result["flights"].append(flight_result_outbound["flights"][0])
    ...
    if isinstance(flight_result_inbound, dict):
        if "flight" in flight_result_inbound and flight_result_inbound["flight"]:
            ...
            merged_flight_result["flights"].append(flight_result_inbound["flights"][0])

    
    # 使用 attraction_seed_agent 获取景点推荐，并取前5个
    ...
    try:
        ...
        if isinstance(view_result, dict) and "attractions" in view_result:
            view_result["attractions"] = view_result["attractions"][:5]
            
    except Exception as e:
        print(f"调用 run_seed_agent 失败: {e}")
        view_result = {}
        
    stored_payload = _build_standard_payload(hotel_request_json, merged_flight_result, hotel_result, view_result)
...
def main() -> None:
    print("示例: 我从吉隆坡去首尔玩，预算10000，一个人，2026.5.1到2026.5.4")
    # 为了自动化测试，不再阻塞等待输入
    user_text = "我从吉隆坡去首尔玩，预算10000，一个人，2026.5.1到2026.5.4"
